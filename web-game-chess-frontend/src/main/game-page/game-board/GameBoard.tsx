...
import { checkAreas } from "./GameBoardControledAreas";
...
function GameBoard({
    ...
    const [whiteControledAreas, setWhiteControledAreas] = useState<number[][]>(
        []
    );
    const [blackControledAreas, setBlackControledAreas] = useState<number[][]>(
        []
    );
    ...
    useEffect(() => {
        ...
        const [whiteControledAreas, blackControledAreas] = checkAreas(matrix);

        setWhiteControledAreas(whiteControledAreas);
        setBlackControledAreas(blackControledAreas);
    }, [gameData]);
    ...
    const displayField = (
        ...
        const coordinates = [
            (fieldCoor % 8) + 1,
            8 - Math.floor(fieldCoor / 8),
        ];

        const isInTipFields = tipFields.some(
            (coordinate) =>
                JSON.stringify(coordinate) === JSON.stringify(coordinates)
        );

        const isInWhite = whiteControledAreas.some(
            (coordinate) =>
                JSON.stringify(coordinate) === JSON.stringify(coordinates)
        );
        const isInBlack = blackControledAreas.some(
            (coordinate) =>
                JSON.stringify(coordinate) === JSON.stringify(coordinates)
        );
        ...
        fields.push(
            <div
                ...
                className={`${classes.field} ${isInTipFields ? classes.tip : ""} ${checkCoordinatesEquality(coordinates, selectedCoor) ? classes["selected-piece"] : ""}`}
                onClick={() => {
    ...
    const mapFromGamePosition = (position: string): JSX.Element => {
        ...
        return (
            <div
                className={`${classes.board__grid} ${playerData.color === pieceColor.black ? classes["black-board"] : classes["white-board"]}`}
            >
                {fields}
            </div>
        );
    ...
    return (
        <div className={classes.board}>
            <div
                className={`${classes.board__rows} ${playerData.color === pieceColor.black ? classes["black-indicators"] : classes["white-indicators"]}`}
            >
            ...
            <div
                className={`${classes.board__columns} ${playerData.color === pieceColor.black ? classes["black-indicators"] : classes["white-indicators"]}`}
            >

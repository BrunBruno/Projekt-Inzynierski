...
function GameBoard({ gameData, gameStates, selectionStates, setSelectionStates, chosePiece }: GameBoardProps) {
  ///
    ...
    if (lastMoveIndex >= 0) {
      ...
      if (
        ...
        if (pieceParent) {
          ...
          performMoveAnimation(innerBoardRef.current, movedPiece, oldCoor, newCoor);
        }
      }
    }
  }, [gameData]);
  ...
  const onSelectField = (
    ...
    if (isInTipFields) {
      performMoveAnimation(outerBoardRef.current, selectionStates.target, selectionStates.coordinates, coordinates);

...
public class DeclineInvitationRequestHandlerTests {
    ...
    public async Task Handle_Should_Remove_Invitation_On_Success() {
        ...
        _mockInvitationRepository.Setup(x => x.GetByGameId(gameId)).ReturnsAsync(invitation);
        ...
        _mockInvitationRepository.Verify(x => x.Delete(invitation), Times.Once);
    }
    ...
    public async Task Handle_Throw_NotFoundException_When_Game_Does_Not_Exist() {
        ...
        _mockInvitationRepository.Verify(x => x.Delete(It.IsAny<Invitation>()), Times.Never);
    }
